...
class DuplexS2SSpeechDecoderModel(LightningModule, HFHubMixin):
    def __init__(self, cfg: dict) -> None:
        ...
        self.source_fps = self.source_sample_rate / (
                    self.source_sample_rate * cfg.data.frame_length)  # conver frame rate in fps
        ...
        if isinstance(llm_tokenizer_vocab_items, dict):
            ...
            llm_tokenizer_vocab_items = [(subword, self.tokenizer.tokenizer._tokenizer.token_to_id(subword)) for subword
                                         in llm_tokenizer_vocab_items]
    ...
    def init_speech_generation_from_another_s2s_checkpoint(self, checkpoint_path):
        if checkpoint_path is not None:
            ...
            checkpoint_state = {k.replace("model.speech_decoder.", "").replace("speech_generation.", ""): v for k, v in
                                checkpoint_state.items() if "model.speech_decoder." in k or "speech_generation." in k}
            checkpoint_state = set_model_dict_for_partial_init(checkpoint_state, self.speech_generation.state_dict())
            self.speech_generation.load_state_dict(checkpoint_state, strict=True)
    ...
    def forward(self, input_embeds: Tensor, cache=None, input_audio_tokens=None, seq_mask=None, target_text_tokens=None,
                modality_adapter_emb=None, asr_emb=None, speaker_encoder_emb=None) -> dict[str, Tensor]:
        """
        ...
        if self.speech_generation.use_input_cache and not self.training:
            ...
            if self.cfg.get("drop_text_bos_prob", None) and random.random() < self.cfg.drop_text_bos_prob:
                target_text_tokens = torch.where(target_text_tokens == self.text_bos_id, self.text_pad_id,
                                                 target_text_tokens)

            if self.cfg.get("drop_text_eos_prob", None) and random.random() < self.cfg.drop_text_eos_prob:
                target_text_tokens = torch.where(target_text_tokens == self.text_eos_id, self.text_pad_id,
                                                 target_text_tokens)

        audio_logits, _ = self.speech_generation(
            out['last_hidden_state'].transpose(0, 1), seq_mask, input_audio_tokens=input_audio_tokens,
            target_text_tokens=target_text_tokens, modality_adapter_emb=modality_adapter_emb, asr_emb=asr_emb,
    ...
    def add_noise_to_batch(self, batch_audio, noise_folder, snr_db=20, noise_prob_scale_user=0.3,
                           noise_prob_scale_user_min_snr=-15, noise_prob_scale_user_max_snr=24, snr_measure_dur=0.0,
                           noise_resample=True, noise_prob_low_pass=0.1):
    ...
    def prepare_inputs(self, batch: dict):
        ...
        if self.cfg.get('use_old_noise_aug', None):
            ...
            if self.training and 's2s_duplex_overlap_as_s2s_duplex' not in batch[
                "formatter"] and noise_prob and random.random() < noise_prob:
                batch["source_audio"] = self.add_noise_to_batch(batch["source_audio"],
                                                                ...
                                                                noise_prob_scale_user_max_snr=24, snr_measure_dur=0.0,
                                                                noise_resample=True, noise_prob_low_pass=0.1)
        else:
            ...
            if self.training and random.random() < self.cfg.get('noise_prob_scale_user', 0.0):
                ...
                scaling_factor = torch.rand(batch["source_audio"].size(0), device=batch["source_audio"].device) * (
                            max_scale_val - min_scale_val) + min_scale_val
            ...
            if self.training and random.random() < self.cfg.get('noise_prob_low_pass', 0.0):
                ...
                batch["source_audio"] = torchaudio.functional.lowpass_biquad(waveform=batch["source_audio"],
        ...
        if not self.training:
            ...
            if self.speech_generation.use_speaker_encoder:
                ...
                speaker_encoder_emb = self.speech_generation.get_speaker_embedding(target_first_turn_audio,
        ...
        if self.cfg.get("scale_loss_by", None):
            if self.cfg.scale_loss_by == 'non_sil_t':
                loss_scale[:, :, :1] = torch.where(text_labels.unsqueeze(-1) != self.text_pad_id, 4.0,
                                                   loss_scale[:, :, :1])
                ...
                loss_scale[:, :, :1] = torch.where(text_labels.unsqueeze(-1) == self.text_bos_id, 12.0,
                                                   loss_scale[:, :, :1])
                loss_scale[:, :, :1] = torch.where(text_labels.unsqueeze(-1) == self.text_eos_id, 12.0,
                                                   loss_scale[:, :, :1])
                ...
                speech_tot_scale_for_valid_tokens = loss_scale[:, :, -1:].flatten(1, 2).sum(
                    -1)  # use only the last speech channel because all the channels are identical
        ...
        if self.cfg.get("debug_dataloader_audios_path", None) and self.training:
            ...
            for i in range(audio_labels_.shape[0]):
                ...
                write_wave(
                    ...
                    os.path.join(self.cfg.get("debug_dataloader_audios_path"),
            ...
            # check text
            print("text_labels decoded:",
                  tokens_to_str(text_labels[-1:], target_codes_lens - 1, tokenizer=self.tokenizer,
                                pad_id=self.text_pad_id))
            print("target labels from dataloader decoded:",
                  tokens_to_str(batch["target_tokens"][-1:], target_codes_lens - 1, tokenizer=self.tokenizer,
                                pad_id=self.text_pad_id))
            print("Number of padding tokens on the begining:",
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        tolerance = int(self.cfg.get("val_acc_tolerance", 160) / (
                    1000 / self.target_fps))  # 160 ms as default tolerance --> 2 tokens for 12.5FPS and 1 for 25FPS
        ...
        if self.cfg.get("eval_behavior", None):
    ...
    def validation_step(self, batch: dict, batch_idx: int):
        ...
        for name, dataset_batch in batch.items():
            ...
            if self.cfg.get("eval_behavior", None):
                self.behavior.update(name=name, pred_audio=resample(results["audio"], 22050, 16000),
                                     user_audio=dataset_batch["source_audio"])
                # Use GT agent speech for debug:
